...
const sendLoginCode = asyncHandler(async (req, res) => {
  ...
  let patientToken = await Token.findOne({
    patientId: patient._id,
    expiresAt: { $gt: Date.now() },
  });
  if (!patientToken) {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: `Access code sent to ${email}` });
  } catch (error) {
    res.status(500);
    throw new Error("Email not sent, please try again");
  }
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  if (patient && passwordIsCorrect) {
    ...
    res
      .status(200)
      .json({ message: "password change succesful, pls re login" });
  } else {
    res.status(400);
...
const sendAccessCodeToPatient = async (
  ...
  patientName,
  accessCode,
  doctorName // Add doctorName parameter
) => {
...
const requestAccessToPatientData = asyncHandler(async (req, res) => {
  ...
  // Get the doctor's name (assuming the doctor model has firstName and lastName fields)
  const doctorName = `${doctor.name}`;

  // Proceed with finding the patient and sending the access code as before
  ...
  // Assign the requesting doctor's ID to the patient's record
  patient.requestingDoctor = loggedInDoctorId;
...
const getPatientFilesByHospitalId = asyncHandler(async (req, res) => {
  ...
  if (!patient) {
    return res
      .status(404)
      .json({ error: "Patient not found for the provided email" });
  }
...
const getPatientFileById = asyncHandler(async (req, res) => {
  ...
  if (!selectedFile) {
    return res
      .status(404)
      .json({ error: "Patient file not found for the provided ID" });
  }
